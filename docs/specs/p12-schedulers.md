# P12: Background Schedulers (16 Schedulers)
...
scheduler/
  ...
  video_task_retention.go # 代理视频任务保留清理 (configurable)
  log_retention.go        # 代理日志保留清理 (legacy fallback, 默认30min)
  oauth_loopback.go       # OAuth Loopback 回调监听器
  oauth_refresh.go        # OAuth Token 自动刷新 (60s, lead times per provider)
```
...
| 13b | **Video Task Retention** | 由 `PROXY_VIDEO_TASK_RETENTION_PRUNE_INTERVAL_MIN` config 决定 (默认 configurable) | — | proxyVideoTaskRetentionService | 清理过期视频任务记录 |
...
| 16 | **OAuth Token Refresh** | 60s (最小 60s) | — | oauth.RefreshAccessTokenSingleflight | 扫描 OAuth 账号, 按 provider lead time (codex=5d, claude=4h, gemini-cli/antigravity=5min) 提前刷新临近过期的 token |
...
### OAuth Token Refresh (Scheduler 16)
- 60s interval (最小 60s)，首次立即执行，passInFlight 串行化
- 扫描 `accounts` × `sites` 中 `oauth_provider IS NOT NULL` 的记录
- 仅刷新符合以下条件的账号：
  - account/site 均为 `active`
  - `extraConfig.oauth.refreshToken` 存在
  - `extraConfig.oauth.tokenExpiresAt - now <= lead time`
- Lead time 按 provider 区分：
  - `codex`: 5 天 · `claude`: 4 小时 · `gemini-cli`: 5 分钟 · `antigravity`: 5 分钟 · 其他: 5 分钟
- 调用 `oauth.RefreshAccessTokenSingleflight`（singleflight 去重）

---

## Acceptance Criteria
- [ ] 16 个 scheduler (+ 13b video retention) 全部实现并在 app.Start 时注册
