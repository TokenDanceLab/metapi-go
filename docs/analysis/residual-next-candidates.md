# Residual next candidates (post v0.8.29 / M40)
...
**Context**: **v0.8.29 shipped** (#423 REL-PREFERRED-BREAKER preferred respects open site/model breaker, #424 REL-COOLDOWN-TS parse CooldownUntil, #425 REL-CONDUCTOR-BUDGET hard attempt budget + nil RefreshAuth failover, #426 residual honesty; PRs #427/#428/#430/#431). Board was clean after M39. **Milestone 40 opened** with client/routing polish board **#433–#435**. Prior **v0.8.28**: #416–#418 (PRs #419–#421). Prior **v0.8.27**: #407–#410 (PRs #411–#414). Prior **v0.8.26**: #397–#400 (PRs #401–#404/#406). Prior **v0.8.25**: #382–#384. M35 closed: #388 review synthesis, **#389/#396** endpoint early reject, **#390/#395** multi-route list regression. Original P0 #405/#565/#515 already-correct in code.  
...
**Active wave**: Milestone 40 / **v0.8.30** board **#433–#435** (client/routing polish; latest release remains **v0.8.29** until release gate)
...
| SEC-OAUTH-NOTIFY-REDIR | OAuth Codex + Telegram notify clients lack CheckRedirect | **active** (#433) | `service/oauth/codex.go` `newOAuthHTTPClient` has no CheckRedirect; `service/notify/telegram.go` bare `&http.Client{Timeout: 30s}` when not using system proxy. RuntimeExecutor / probe / harness / defaultUpstreamClient already share `RejectCrossOriginRedirect` (#357/#416) | **M40 P0 #433** — both clients set CheckRedirect to `platform.RejectCrossOriginRedirect` (or shared helper); public origin 302 → different host must error | Residual bare OAuth/notify redirect SSRF |
| REL-SOURCE-MODEL | loadRouteMatch source-model fallback stub always empty | **active** (#434) | `routing/selector.go` builds `fallbackSourceModelByRouteID` but always sets `""` (TODO); group/source routes with blank channel SourceModel never get pattern fallback for eligibility / mapping / health model resolution | **M40 P1 #434** — load source route model patterns and apply fallback into candidate channel source model | Group route eligibility/mapping miss on empty SourceModel |
...
| SEC-REDIR | RuntimeExecutor CheckRedirect SSRF | **present** (RuntimeExecutor #357/#360; bare clients #416/#420); residual OAuth/notify via **SEC-OAUTH-NOTIFY-REDIR #433** | `rejectCrossOriginRedirect` / shared `RejectCrossOriginRedirect` on RuntimeExecutor client + platform DoWithProxy **and** residual bare clients: `channel_health_probe.go`, `channel_test_harness.go`, `defaultUpstreamClient` (#416/#420). Regression: public origin 302 → 169.254/loopback must error and not fetch target body. **Still residual (M40):** Codex OAuth + Telegram notify clients (#433) | Done for RuntimeExecutor + bare probe/harness/defaultUpstreamClient; OAuth/notify active #433 | Residual novel client constructors only if product AC |
...
2. **Active M40 board (#433–#435)** — Enterprise residual client/routing polish **v0.8.30** (latest tag stays **v0.8.29** until release gate):
   1. **#433 SEC-OAUTH-NOTIFY-REDIR (P0)**: Codex OAuth HTTP client + Telegram notify client share `RejectCrossOriginRedirect`; public origin 302 → different host must error.
   2. **#434 REL-SOURCE-MODEL (P1)**: `loadRouteMatch` source-model fallback from source route pattern (no empty stub) for eligibility/mapping/health.
   3. **#435 docs**: this residual honesty pass + MASTER M40 pointer (no product code).
3. **REL-PREFERRED-BREAKER** / **REL-COOLDOWN-TS** / **REL-CONDUCTOR-BUDGET** fully **present**. **SEC-REDIR** present for RuntimeExecutor + bare probe/harness/defaultUpstreamClient; residual OAuth/notify clients active as **SEC-OAUTH-NOTIFY-REDIR #433**. **SEC-MONITOR** fully **present** for opaque session + logout clear (rotate/invalidate on auth_token change only with product AC). **SEC-AUTH-TIMING** fully **present**. **CTX-520** stays **present-with-residual** (OpenAI + Claude max_tokens reject shipped; further dialects / null-context residual). **P0-555** stays **present-with-residual** (warn shipped; provider-ignore / media / lag / orphan join residual).
...
- Claiming Codex OAuth or Telegram notify clients reject cross-origin redirects until #433 lands.
- Claiming `loadRouteMatch` applies source-route model-pattern fallback for empty channel SourceModel until #434 lands.
...
- Milestone: [Enterprise residual client/routing polish v0.8.30](https://github.com/TokenDanceLab/metapi-go/milestone/40) · board **#433–#435**
