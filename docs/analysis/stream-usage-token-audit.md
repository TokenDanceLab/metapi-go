...
| OpenAI `stream_options.include_usage` not forced | **Addressed for chat + legacy completions stream (#345/#350)**: `applyUpstreamStreamIncludeUsage` injects `include_usage=true` on OpenAI-compatible chat/completions and `/v1/completions` stream bodies; skips codex/sub2api and non-OpenAI-stream-options paths. Still residual if provider ignores the flag or never emits usage. |
