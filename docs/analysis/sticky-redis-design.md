...
| Request-time timeout / network / RESP error on `GET` | Treat as miss → fall through to local map, then normal selection if still empty. **Never** fail the proxy request solely because sticky store is down (fail-open; sticky map is **not** a hard dependency). |
...
## Selection revalidation (still needed under a shared sticky map)

Even with a perfect shared-map hit, `SelectProxyChannelForAttempt` must keep existing guards:
