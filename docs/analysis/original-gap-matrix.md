...
| 577 | AnyRouter check-in and model list broken | ops-checkin | partial | `platform/anyrouter.go` embeds `NewApiAdapter` (inherits Checkin/GetModels); token APIs intentionally disabled. **Unit**: `service/checkin/anyrouter_test.go` exercises adapter checkin + user discovery against httptest. Live AnyRouter model-list/check-in still residual. | P3 | yes |
| 571 | Codex OAuth cannot call gpt-5.5 | feature-protocol | unknown-needs-runtime | Static: gpt-5 family allowlist + seed/probe prefer **gpt-5.5** (`service/oauth/codex_models.go`); unit tests `codex_models_test.go` lock allow/seed/probe. Discovery soft-retry 30s/45s (#489). Still needs live OAuth+cloud model probe to flip present. | P1 | yes |
...
| 555 | Token usage statistics inaccurate | bug-correctness | **present-with-residual** | Aggregation + stream merge honesty + Gemini later-wins + media detail fold + **orphan projection observability** (`OrphanLogs`/slog; watermark advances). Residual: multi-instance lag / perfect billing. | P0 | yes |
